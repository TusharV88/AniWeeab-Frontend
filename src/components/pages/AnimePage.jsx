...
import { useLazyAnimeDataQuery, useLazySeasonsNowMainQuery } from "../../redux/services/anime.js";
import { useDispatch, useSelector } from "react-redux";
import { getToken } from "../../utils/userToken.js";
import { useEffect, useState } from "react";
import { incSeasonsAnimeMain } from "../../redux/slices/Anime/SeasonsNowMain/seasonsNowMainLimit.js";
import { Box, Button, CircularProgress, Grid, Pagination, Stack, useMediaQuery, useTheme } from "@mui/material";
import { toast } from "react-toastify";
import { initSeasonsNowAnimeMain } from "../../redux/slices/Anime/SeasonsNowMain/seasonsNowMainSlice.js";
...
const AnimePage = () => {
    ...
    const [animeTrigger] = useLazyAnimeDataQuery();


    const seasonsMainData = useSelector((state) => state.animeSeasonsNowMain);
    ...
    const seasonsMainLimit = useSelector((state) => state.animeSeasonsNowMainLimit);
    ...
    useEffect(() => {
        ...
        if (seasonsMainLimit === 0) {
            dispatch(incSeasonsAnimeMain());
            seasonMainTrigger(token)
                .unwrap()
                .then((fulfilled) => {
                    if (fulfilled !== undefined) {
                        dispatch(initSeasonsNowAnimeMain(fulfilled.seasons));
                    }
                })
                .catch((rejected) => {
                    toast.error(
    ...
    useEffect(() => {
        if (mediaLimit === 0 && seasonsMainStateLength > 0) {
            dispatch(incMedia());
            animeTrigger({ token, mediaState, pageNo })
                .unwrap()
                .then((fulfilled) => {
                    if (fulfilled !== undefined) {
